...
int n ;
///-------------------------------------------------GEO END-----------------------------------
double getRes(vector<PT>convex,int i){
        int N  = sz(convex) ;
        double a,b,c ;
        getEquations(convex[i], convex[(i + 1) % N], a, b, c);
        double ret = fabs(a * sum1 + b * sum2 + c * n) / sqrt(a * a + b * b);
        return ret ;
}
int main(){
