...
#define MAXN 15
vector<PT> conerPoints(PT Centre,double w,double h,double angle){
...
int main(void){
    ...
    while(testcases--){
            ...
            vector<PT>Polygon ;
            rep(i,0,N){
                cin>>centre.x>>centre.y>>h>>w>>angle ;
                coveredArea+=(h*w) ;
                vector<PT>Points = conerPoints(centre,w,h,angle) ;
                rep(i,0,sz(Points)){
                  Polygon.pb(Points[i]) ;
                }
            }
            vector<PT>convexHall = ConvexHull(Polygon) ;
            //cout<<convexHall<<endl ;
            double res=coveredArea/ComputeArea(convexHall) ;
            res*=100 ;
